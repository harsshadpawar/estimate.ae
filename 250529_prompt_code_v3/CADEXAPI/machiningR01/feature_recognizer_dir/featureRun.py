...
# Your other imports
from CADEXAPI.machiningR01.feature_recognizer_dir import feature_recognizer
...
def parse_feature_recognizer_output(output_text):
    ...
    while line_index < len(lines):
        ...
        if feature_match and current_part:
            ...
            # Check if the next line doesn't have 8 spaces indentation (meaning no details)
            # or if this is the last line, or if the next line is another feature type
            next_line_index = line_index + 1
            if (next_line_index >= len(lines) or 
                not re.match(r'\s{8}', lines[next_line_index]) or 
                re.match(r'\s{4}[A-Za-z]', lines[next_line_index])):
                # This feature has no detailed entries, add a single summary entry
                feature_group = {
                    "count": feature_count,
                    "properties": {}
                }
                current_part["features"][feature_type].append(feature_group)
            
            line_index += 1
            continue
        
        # Extract feature details (8 spaces indentation)
        detail_match = re.match(r'\s{8}(\d+)(.+?)(?:\(s\) with|\s+)', line)
        if detail_match and current_part and current_feature_type:
            ...
            detail_type_raw = detail_match.group(2).strip() if detail_match.group(2) else ""
            
            # Clean up the detail type - remove "with" if present
            detail_type = detail_type_raw.replace("with", "").strip()
            ...
            # Add the detail_type if it's not empty and not just "with"
            if detail_type and detail_type != "with":
            ...
            if "with" in line:
                ...
                while line_index < len(lines):
                    ...
                    if (re.match(r'\s{8}(\d+)', next_line) or 
                        re.match(r'\s{4}[A-Za-z]', next_line) and 
                        not re.match(r'\s{10}', next_line)):
                        break
    ...
    # Post-processing: handle empty feature arrays
    for part in result["parts"]:
        for feature_type, features in part["features"].items():
            if not features:
                # If a feature type has no detailed entries, add a summary entry
                part["features"][feature_type] = [{
                    "count": sum(f["count"] for f_type, f_list in part["features"].items() 
                              for f in f_list if f_type == feature_type),
                    "properties": {}
                }]
...
def capture_and_process_output(source_file, operation):
    ...
    with redirect_stdout(buffer):
        ...
        feature_recognizer.main(source_file, operation)
    ...
    # print(output_text)
    file1 = "./outputs/CDEX/FR/"
    if not os.path.exists(file1):
        os.makedirs(file1)
    # Save the output to a text file
    with open(file1+"feature_recognition_output.txt", "w") as f:
        f.write(output_text)
    ...
    for part in structured_data["parts"]:
        ...
        # 2. Clean up any "(es) with" suffix from type names
        for feature_type, features in part["features"].items():
            for feature in features:
                if "type" in feature and "with" in feature["type"]:
                    # Clean up the type name, removing '(es) with' or similar patterns
                    feature["type"] = re.sub(r'\(es\)\s+with$', '', feature["type"]).strip()
        
        # 3. Fix Boss features that might be misplaced
        # Look for features with Boss type
        boss_features = []
        wrong_category = None
        
        # Search for misplaced Boss features
        for feature_type, features in part["features"].items():
            for feature_idx, feature in enumerate(features):
                if "type" in feature and "Boss" in feature["type"]:
                    # Found a Boss feature in wrong category
                    boss_features.append(feature)
                    # Keep track of which category and index to remove it from
                    if wrong_category is None:
                        wrong_category = (feature_type, feature_idx)
        
        # If we found misplaced Boss features, create a Boss category and move them there
        if boss_features:
            # Create the Boss category if it doesn't exist
            if "Boss" not in part["features"]:
                part["features"]["Boss"] = []
            
            # Add the Boss features to the correct category
            for feature in boss_features:
                # Clean up the type
                if "type" in feature:
                    if "Boss" in feature["type"]:
                        # If type contains Boss, clean it up or remove it
                        feature["type"] = "Boss"
                part["features"]["Boss"].append(feature)
            
            # Remove the Boss features from the wrong category
            if wrong_category:
                feature_type, start_idx = wrong_category
                # Remove the boss features from the wrong category
                part["features"][feature_type] = [
                    f for i, f in enumerate(part["features"][feature_type])
                    if "type" not in f or "Boss" not in f.get("type", "")
                ]
        
        # 4. Add entries for empty feature arrays (features without detailed properties)
    ...
    json_file_path = "./outputs/CDEX/FR/feature_recognition.json"
...
def main(step_file_path, machining_process):
    """
    Main function to run the feature recognizer and process its output.
    """
    # Initialize the MTK license
    aKey = license.Value()
    ...
    # Run the feature recognizer and capture its output
    structured_data = capture_and_process_output(step_file_path, machining_process)
    
    return structured_data


#     # Define file paths and operation types
#     aSource = abspath(dirname(Path(__file__).resolve()) + "/../../../models/shaft_5x.step")
#     anOperation = "turning"
