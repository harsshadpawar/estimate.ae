...
#     existing_limits = db.query(PlanLimit).count()
...
#     # Map plan name to id
#     plan_ids = {plan.name.value.lower(): plan.id for plan in plans}

#     # Define limits
#     limits = [
#         # Free plan
#         PlanLimit(subscription_plan_id=plan_ids.get("free"), period="daily", api_name="dfm_analysis", limit_count=3),
#         PlanLimit(subscription_plan_id=plan_ids.get("free"), period="monthly", api_name="download_report", limit_count=10),
#     ]

#     # Filter out any limits with None (in case plan was missing)
#     limits = [limit for limit in limits if limit.subscription_plan_id is not None]
...
#     # Step 1: Prepare the plan name → id mapping
...
#     # Step 2: Define country-specific pricing
#     pricing_matrix = {
#         "US": {
...
#     # Step 3: Prepare existing (plan_id, country_code) pairs to avoid duplicates
...
#     # Step 4: Construct raw insert statements
#     inserts = []
...
#     # Step 5: Bulk insert using raw SQL
#     if inserts:
#         db.execute(
#             text("""
...
# from sqlalchemy import text
# from app.models.subscription_plan_model import PlanName  # Ensure you import the Enum correctly

def seed_country_subscription_plans(db: Session):
    # Step 1: Prepare the plan name → id mapping using Enum values
    plan_result = db.execute(text("""
        SELECT name, id FROM subscription_plans
        WHERE name IN (:free, :premium, :enterprise)
    """), {
        'free': PlanName.free.value,         # Pass the string value of the enum
        'premium': PlanName.premium.value,   # Pass the string value of the enum
        'enterprise': PlanName.enterprise.value  # Pass the string value of the enum
    }).fetchall()
