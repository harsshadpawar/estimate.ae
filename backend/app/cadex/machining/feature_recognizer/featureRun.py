...
mtk_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "..", "MTKConverter")
)
if mtk_path not in sys.path:
    sys.path.insert(0, mtk_path)

import manufacturingtoolkit.CadExMTK as mtk
from  app.cadex.machining.feature_recognizer.feature_recognizer import main 


# Define file paths and operation types
aSource = abspath(dirname(Path(__file__).resolve()) + "/BLOCK.stp")
anOperation = "milling"
