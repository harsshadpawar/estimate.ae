...
import uuid
from app.database.init_db import seed_roles, seed_subscription_plans,seed_plan_limits, seed_countries, seed_country_subscription_plans
...
# ✅ Limiter setup
# def key_func_jwt(request: Request):
#     return decode_jwt(request)["sub"]

# limiter = Limiter(key_func=key_func_jwt)
# app = FastAPI(title=settings.APP_NAME)
# app.state.limiter = limiter
# app.add_exception_handler(RateLimitExceeded, _rate_limit_exceeded_handler)

# ✅ Optional: skip admin from rate limit
# @limiter.exempt_when
# def skip_admin(request: Request):
#     return decode_jwt(request)["role"] == "admin"
...
async def startup_event():
    ...
    # for db logging
    # save_log(db, level="INFO", message="🚀 FastAPI app has started", endpoint="/startup")
