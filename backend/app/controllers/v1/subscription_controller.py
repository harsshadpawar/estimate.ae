...
from app.services.subscription_service import (
    ...
    update_subscription,
    delete_subscription,
    get_all_country_subscription_plans
)
...
@router.get("/country-subscription-plans")
def fetch_country_subscription_plans(db: Session = Depends(get_db), user_payload=Depends(require_roles_from_token("super-admin"))):
    try:
        plans = get_all_country_subscription_plans(db)
        if not plans:
            raise NotFoundException("No Country subscription plans found")
        
        return success_response(
            data=jsonable_encoder(plans),
            message="Country subscription plans fetched successfully",
            status_code=200
        )
    except NotFoundException as e:
        return error_response(str(e), status_code=404)    
    except Exception as e:
        logger.error("Error fetching subscription plans", exc_info=True)
