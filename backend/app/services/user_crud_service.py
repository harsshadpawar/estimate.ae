...
from app.models.user_model import User, UserCreate, UserUpdate, UserAllOut
from uuid import uuid4
from typing import List, Optional
from fastapi import HTTPException, status
from app.main import logger
from app.exceptions.custom_exceptions import AlreadyExistsException, NotFoundException, BadRequestException
...
from app.utils.validators import validate_otp_code
from app.utils.serialize import serialize_value
from sqlalchemy.sql import text
from app.constants.constants import RESTRICTED_ASSIGNABLE_ROLES
...
def verify_otp_service(data: VerifyOtpRequest, db: Session):
    try:
        validate_otp_code(data.otp_code)
        otp_query = (
            db.query(OTP)
            .join(User)
            .filter(
...
def get_all_users_detailed_sql(db: Session):
    sql = text("""
        SELECT 
            u.id,
            u.first_name,
...
def update_user(user_id: str, payload: UserUpdate, db: Session) -> Optional[User]:
...
def assign_role_to_user(user_id: str, role_name: str, db: Session):

    if role_name.lower() in map(str.lower, RESTRICTED_ASSIGNABLE_ROLES):
        raise ValueError(f"Assignment of '{role_name}' role is not allowed")
    
    user = db.query(User).filter(User.id == user_id, User.is_deleted == False).first()
    if not user:
        return None

    role = db.query(Role).filter(Role.name == role_name, Role.name != RESTRICTED_ASSIGNABLE_ROLES[0]).first()
    if not role:
